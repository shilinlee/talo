...
不要把远端 workspace 当成真正的代码仓库。它只是一个可以删除、重建、覆盖的镜像。
...
- 命令以 0 退出。
- 输出表明同步完成。
- 远端 workspace 位于 `$remote_base/.talo/workspaces/<project>` 之下。
...
1. 本地文件是权威源。
2. 远端 workspace 是镜像，不是源码仓库。
3. 不要依赖远端 `.git`；同步会有意排除 git 元数据。
4. 本地修改后、远端测试前必须同步。
